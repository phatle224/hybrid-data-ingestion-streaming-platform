...
### Bước 4: Chuyển Đổi Transform Layer Sang dbt (Data Build Tool) (Đã Hoàn Thành - DONE)
Dự án dbt đã được tái cấu trúc thành công và chạy ổn định trên database `insuranceWarehouse` với các schema được phân tách rõ ràng:
...
4.  **Tầng Core Warehouse (`models/marts/warehouse/`):**
    *   `dim_customers.sql`: Chiều khách hàng (tính toán sẵn nhóm tuổi, chuẩn hóa thông tin cá nhân).
    *   `fct_contracts_wide.sql`: Bảng sự kiện rộng hợp đồng được join sẵn từ các bảng chiều và bảng dữ liệu sạch.
5.  **Tầng Data Mart (`models/marts/mart/`):**
    *   `dm_profiling_analysis.sql`: Data Mart phân tích rủi ro bồi thường và hành vi claims (tính toán tỷ lệ đền bù, thời gian xảy ra claim từ lúc ký hợp đồng, nhóm bệnh lý thông thường).
