...
flowchart LR
    subgraph "Databases"
        STG["🗄️ insuranceWarehouse<br/>(stgClaim, stgContract)"]
        RPT["🗄️ insuranceReporting<br/>(profiling_analysis)"]
    end
...
| **CDC Portal** | frontend + backend | Upload Excel offline data | Excel files | insuranceWarehouse.stgContractObjectOffline |
