...
class ProfilingConsumer(BaseKafkaConsumer):
    ...
    def __init__(self):
        # ── Configuration ────────────────────────────────────
        topic_prefix = os.getenv('TOPIC_PREFIX', 'staging.insuranceWarehouse')
    ...
    def _upsert_profiling_record(self, record: Dict[str, Any]) -> bool:
        ...
        query, values = SQLQueryBuilder.build_reporting_upsert(
            table='insuranceReporting.profiling_analysis',
            data=record,
            key_fields=['id'],
            exclude_fields=[],
        )
    ...
    def _delete_profiling_record(self, claim_id: str) -> bool:
        ...
        try:
            self._reporting_db.execute(
                "DELETE FROM insuranceReporting.profiling_analysis WHERE id = %s",
                [claim_id]
            )
            return True
        except Exception:
