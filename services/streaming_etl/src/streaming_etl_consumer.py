...
Architecture:
    Debezium → Kafka (staging topics) → This Consumer → insuranceReporting

Topics consumed:
    - staging.insuranceWarehouse.stgContract
    - staging.insuranceWarehouse.stgContractObject (HEALTH)
    - staging.insuranceWarehouse.stgContractObjectVehicle
    - staging.insuranceWarehouse.stgContractObjectTravel
    - staging.insuranceWarehouse.stgContractObjectMoto
    - staging.insuranceWarehouse.stgContractObjectSocialInsurance
    - staging.insuranceWarehouse.stgContractObjectMedicalInsurance
    - staging.insuranceWarehouse.stgContractObjectHouse
    - staging.insuranceWarehouse.stgContractObjectOffline (Excel uploads)
...
class StreamingETLConsumer(BaseKafkaConsumer):
    ...
    def __init__(self):
        # ── Configuration ────────────────────────────────────
        self._topic_prefix = os.getenv('STAGING_TOPIC_PREFIX', 'staging.insuranceWarehouse')

        self._staging_config = MySQLConfig(database='insuranceWarehouse', autocommit=True).get_config()  # autocommit=True: tránh REPEATABLE READ ẩn row mới
        self._reporting_config = MySQLConfig(database='insuranceReporting').get_config()
    ...
    def _has_online_duplicate(self, data: Dict[str, Any]) -> bool:
        ...
        result = self._reporting_db.fetch_one(
            """SELECT 1 FROM insuranceReporting.contract
    ...
    def _handle_delete(self, topic_cfg: TopicConfig, data: Dict[str, Any]):
        """
        Propagate a delete event from staging to insuranceReporting.contract.
        ...
        if topic_cfg.is_contract_master:
            ...
            rows = self._reporting_db.execute_with_rowcount(
                "DELETE FROM insuranceReporting.contract WHERE contractId = %s",
                [contract_id],
            )
            self.logger.info(
            ...
            if not contract_object_id:
                return
            rows = self._reporting_db.execute_with_rowcount(
                "DELETE FROM insuranceReporting.contract WHERE contractObjectId = %s",
                [contract_object_id],
            )
            self.logger.info(
    ...
    def _remove_conflicting_offline(self, record: Dict[str, Any]):
        ...
        rows = self._reporting_db.execute_with_rowcount(
            """DELETE FROM insuranceReporting.contract
    ...
    def _upsert_to_reporting(self, record: Dict[str, Any]) -> bool:
        """Upsert record to insuranceReporting.contract."""
        query, values = SQLQueryBuilder.build_reporting_upsert(
            table='insuranceReporting.contract',
            data=record,
